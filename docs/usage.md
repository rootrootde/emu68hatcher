...
3. **Kickstart tab - pick the Workbench version to install.** Select the folder containing your ROMs. emu68hatcher will automatically look in the same folder for the Workbench ADF files, but you can also point it somewhere else. ROMs and ADFs are identified by hash, so the filenames don't matter.
...
5. **Packages tab - enable/disable optional packages and pick the icon set** (GlowIcons on 3.2+, or Standard). Mandatory system packages are always included and not shown here. The first build downloads everything (with caching, subsequent builds are faster).
