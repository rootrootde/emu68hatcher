...
Function .onInit
  ...
  ;detect previous install via the per-app InstDir reg key written below; if found, run its uninstaller silently before installing the new version (avoids orphan files from removed builds)
  ReadRegStr $0 SHCTX "Software\${app_name}" ""
  ${If} $0 != ""
    ExecWait '"$0\uninstall.exe" /$MultiUser.InstallMode /S _?=$0'
  ${EndIf}
