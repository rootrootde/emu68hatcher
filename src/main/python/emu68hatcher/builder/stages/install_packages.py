...
def stage_install_packages(workflow: BuildWorkflow) -> None:
    """install selected packages to the disk image using CSV rules"""
    ...
    workflow._log("Installing packages")

    if not workflow.state.extracted_paths and not workflow.state.downloaded_files:
        workflow._update_state(progress=100.0)
        workflow._log("No packages to install")
        return

    # get extracted packages directory
    extracted_dir = workflow.state.extracted_dir if workflow.state.extracted_dir else (workflow.state.work_dir / "extracted")

    # get local packages directory
    local_packages_dir = Path(__file__).parent.parent.parent / "data" / "local_packages"
    ...
    # get all packages to install (user-selected + mandatory + network stack)
    ...
    if workflow.config.network_stack:
        ...
        if stack_name not in [n.lower() for n in user_packages]:
            user_packages.append(stack_name)
    mandatory = installer.get_mandatory_packages()
    all_packages = list(set(user_packages + mandatory))

    workflow.logger.info(f"Installing {len(all_packages)} packages using CSV rules")
    ...
    for package_name in all_packages:
        ...
        workflow._log(f"Installing {package_name}")

        # install using CSV rules
    ...
    # apply any script modifications queued by the installer
    workflow._update_state(progress=95.0)
    workflow._log("Applying script modifications")
    script_mods = installer.apply_script_modifications()
    if script_mods > 0:
        workflow.logger.info(f"Applied {script_mods} script modifications")

    workflow.state.files_copied += files_installed
    workflow._update_state(progress=100.0)
    workflow._log(f"Installed {total} packages ({files_installed} files)")
